...
export function storeSnapshot(key, value) {
  key = `${key}`;
...
export function normalizeImage(value, detail) {
...
export function generatedImageItems(value) {
  ...
  return [normalizeImage(value), ...(hint === undefined ? [] : [{ type: "input_text", text: hint }])];
}
...
export function decodeAudio(url) {
...
export function wavDuration(url) {
  const bytes = decodeAudio(url);
  if (!bytes || bytes.length < 12) return;
  const id = (offset, size = 4) => String.fromCharCode(...bytes.subarray(offset, offset + size));
  if (id(0) !== "RIFF" || id(8) !== "WAVE") return;
  const view = new DataView(bytes.buffer, bytes.byteOffset, bytes.byteLength);
  let rate, align;
  for (let offset = 12; offset + 8 <= bytes.length;) {
    const kind = id(offset);
...
export function guestValueHelpers() {
  return [stringify, storeSnapshot, normalizeImage, normalizeAudio, generatedImageItems, decodeAudio, wavDuration]
