...
describe("subscription relay placement", () => {
  ...
  it("passes the subscription WebSocket upgrade and 101 through the owner relay", async () => {
    const switched = { status: 101, webSocket: { accept() {} } } as unknown as Response;
    const fetch = vi.fn(async (out: Request) => {
      ...
      expect(out.headers.get("authorization")).toBe("Bearer synthetic-token");
      return switched;
    });
    const namespace = { idFromName: vi.fn((id: string) => id), get: vi.fn(() => ({ fetch })) };
    const response = await routeChatGpt(new Request("https://chatgpt.com/backend-api/codex/responses", {
      headers: { upgrade: "websocket", authorization: "Bearer synthetic-token" },
    }), "owner-1", { CHATGPT_EGRESS: namespace as unknown as DurableObjectNamespace });
    expect(response).toBe(switched);
