...
impl CodeModeHost for JavaScriptCodeModeHost {
    ...
    fn execute_tool<'a>(
        ...
        Box::pin(async move {
            ...
            // This bit is generated by the trusted JS host after checking an
            // unforgeable object identity. It is not part of ToolOutputWire:
            // arbitrary tool payloads/metadata and process wires cannot opt in.
            // ToolOutputWire contains RawValue fields, which require the raw
            // JSON deserializer. Flattening it into another struct makes serde
            // visit those fields through a map and rejects valid metadata as a
            // "newtype struct". Decode the trusted host marker independently.
            ...
            #[derive(Deserialize)]
            struct HostedToolMarker {
                #[serde(default)]
                trusted_unreal_pending: bool,
            }
            let marker = serde_json::from_str::<HostedToolMarker>(&encoded).map_err(|error| {
                CodeModeHostError::new(format!(
                    "JavaScript tool host returned invalid execution JSON: {error}"
                ))
            })?;
            let output = ToolOutput::from_wire(wire).map_err(|error| {
                CodeModeHostError::new(format!("JavaScript tool result was invalid: {error}"))
            })?;
            Ok(if marker.trusted_unreal_pending {
                output.with_trusted_unreal_pending()
            } else {
                output
            })
        })
