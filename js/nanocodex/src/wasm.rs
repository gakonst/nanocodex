...
impl CodeModeHost for JavaScriptCodeModeHost {
    ...
    fn execute_tool<'a>(
        ...
        Box::pin(async move {
            ...
            #[derive(Deserialize)]
            struct HostedToolOutput {
                #[serde(flatten)]
                wire: ToolOutputWire,
                #[serde(default)]
                trusted_unreal_pending: bool,
            }
            let hosted = serde_json::from_str::<HostedToolOutput>(&encoded).map_err(|error| {
                CodeModeHostError::new(format!(
                    "JavaScript tool host returned invalid execution JSON: {error}"
                ))
            })?;
            let output = ToolOutput::from_wire(hosted.wire).map_err(|error| {
                CodeModeHostError::new(format!("JavaScript tool result was invalid: {error}"))
            })?;
            Ok(if hosted.trusted_unreal_pending {
