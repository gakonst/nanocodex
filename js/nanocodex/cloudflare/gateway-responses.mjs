...
const MODELS = ["@cf/zai-org/glm-5.3", "gpt-6-astra", "gpt-6-sol", "gpt-6-luna", "gpt-5.6-sol", "gpt-5.6-terra", "gpt-5.6-luna", "kimi-k3", "mimo-v2.6-pro"];
  ...
  return Object.freeze({ apiBaseUrl, stateless: true,
    async createResponse(endpoint, sessionId, request) {
      ...
      try {
        ...
        let reasoningMode = body.reasoning?.mode;
        if (Array.isArray(body.input)) for (const item of body.input) {
          if (item.type === "configuration_update" && item.reasoning?.mode !== undefined) reasoningMode = item.reasoning.mode;
        }
        if (reasoningMode !== undefined && !["standard", "pro"].includes(reasoningMode)) fail("unsupported reasoning mode");
        const supportsMode = ["gpt-6-sol", "gpt-6-luna", "gpt-5.6-sol", "gpt-5.6-terra", "gpt-5.6-luna"].includes(model);
        if (reasoningMode === "pro" && (provider !== "cloudflare" || !supportsMode)) fail("pro reasoning requires a supported Responses model");
