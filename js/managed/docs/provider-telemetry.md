...
These modules do not register a cron, deploy, or issue requests unless explicitly invoked with `enabled: true`. Default integration must remain disabled. No live paid probe has been performed as part of this implementation.
...
`node --experimental-transform-types --test js/managed/test/provider-telemetry.test.mjs` on Node 22 verifies disabled operation, deterministic-clock latency, censoring, allowlisted endpoints, request budget ordering, privacy projection, and real SQLite persistence/retention. The tests inject fetch and issue no provider calls.
