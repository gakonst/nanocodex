...
try {
  ...
  assert.deepEqual(custom.elements.map(el=>el.text),['Log in']);
  await actBrowserVault(cdp,request,{action:'click',snapshot_id:custom.snapshot_id,ref:custom.elements[0].ref});
  ...
  await assert.rejects(actBrowserVault(cdp,request,{action:'click',snapshot_id:changed.snapshot_id,ref:'e1'}), /safely/);
