...
export function gatewayRuntime(env: GatewaySecrets, route: Pick<ThreadRoute, "backend" | "model" | "provider_model" | "thinking"> | undefined, assertActive: () => void,
  ...
  if (route.backend === "cloudflare") {
    if (!(route.model === "gpt-6-sol" || route.model === "gpt-6-luna" || route.model === "gpt-6-astra" || route.model === "gpt-5.6-sol" || route.model === "gpt-5.6-terra" || route.model === "gpt-5.6-luna") || !gatewayAvailability(env).cloudflare) throw new Error("Pinned cloudflare route requires its configured transport and frontier gate");
