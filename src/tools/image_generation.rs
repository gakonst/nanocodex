...
const DESCRIPTION: &str = include_str!("image_generation/imagegen_description.md");
...
async fn request_for_args(args: &ImagegenArgs, history: &[Value]) -> Result<ImageRequest, String> {
...
fn recent_images(history: &[Value], count: usize) -> Vec<ImageUrl> {
    ...
    for item in history {
        match item.get("type").and_then(Value::as_str) {
            ...
            Some("custom_tool_call") => {
                if let Some(call_id) = item.get("call_id").and_then(Value::as_str) {
                    custom_tool_call_ids.insert(call_id);
                }
            }
            _ => {}
        }
        ...
        match item.get("type").and_then(Value::as_str) {
            Some("message") => {
                if let Some(content) = item.get("content").and_then(Value::as_array) {
                    image_urls.extend(content.iter().rev().filter_map(input_image_url));
                }
            }
            Some("function_call_output")
                if item
                    ...
                    .is_some_and(|call_id| function_call_ids.contains(call_id)) =>
            {
                image_urls.extend(output_image_urls(item));
            }
...
fn output_image_urls(item: &Value) -> impl Iterator<Item = String> + '_ {
    item.get("output")
        .and_then(Value::as_array)
        .into_iter()
        .flatten()
        .rev()
        .filter_map(input_image_url)
}

fn input_image_url(item: &Value) -> Option<String> {
