...
- macOS: `testNativeDockingRetainsEditorsAndRendersGlass` and `testNativeWorkspaceRenderingAndInteractionLatency` passed. A private pane-motion test also passed in this run; it was later removed during the low-signal test cleanup. Light/dark screenshots are `native-glass-splits-light.png` and `native-glass-splits-dark.png`; logs are `desktop-after.log` and `desktop-motion-test.log`.
