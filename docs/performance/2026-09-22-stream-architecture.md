...
- Full Hand sharing: the stale observer error-message assertion now matches the current CLI while retaining the zero-publisher assertion. Both Linux integration tests pass against the built binary, including shared leases across account keys, command execution, lease expiry/reconnect, identity preservation and shutdown without inherited-pipe hangs.
