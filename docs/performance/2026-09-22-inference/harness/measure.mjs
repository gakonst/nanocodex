...
try{
 ...
 for(const transport of transports){
  const resource=await create(transport);let ws,sse;try{
   ...
   let last;for(const label of ['cold','warm1','warm2','cancel']){last=await runTurn(resource,{ws,sse},transport,transport+'_'+label,label==='cancel');if(last.error&&last.error.kind!=='unexpected_turn_outcome')break;}
