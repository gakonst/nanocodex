...
The Linux SSH bootstrap installs the same single daemon with its VM recipe. Older separate factory services must be removed explicitly before installing it. There is one current wire contract: no capability negotiation or legacy metadata fallback. Upgrade the broker and publishers together. There is no new process recovery API or persistence across daemon crashes.
