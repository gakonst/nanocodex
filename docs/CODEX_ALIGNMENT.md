...
A live Astra Code Mode run executed concurrent shell calls (peak concurrency three), calculated 42 from their outputs, and published a validated artifact. A separate live Astra conversation recovered an exact marker across two durable turns with shutdown and reopen between them.

On September 6, the canonical worktree `pnpm dev` stack was also exercised through the account browser UI and local Wrangler Workers, using the existing development SMS fixture and the normal ChatGPT connection flow. Astra calls, account-session authorization, the private egress broker, managed turns, and durable storage were real. The browser defaulted to Astra/low, ran concurrent shell calculations, yielded/resumed Code Mode, and recovered the transcript and an exact workspace file across a full browser reload and idle runtime shutdown. A further reload while a 15-second Code Mode cell was running reconnected and displayed its successful result without repeating the file write. Provider usage reported cache hits on follow-on requests.

Terminal cards render Code Mode text arrays as readable summaries. The test used no provider secrets in browser tooling. Application console and Worker/socket logs were inspected. Rebuilding WASM and cleaning/rebuilding terminal assets while Vite was running caused temporary reload and connection warnings; the final full reload had no application-origin warnings or errors. Wallet extension warnings were unrelated to the application. The Vite development document has no CSP header, so this run does not establish production CSP behavior or real SMS delivery.
