...
The optional subagent extension now exposes the direct-only `collaboration` namespace: `spawn_agent`, `send_message`, `followup_task`, `list_agents`, `wait_agent`, and `interrupt_agent`. Named task paths resolve relative to the caller or from `/root` and remain fenced to their root family. Spawn inherits all safe history by default; `fork_turns: "none"` and positive integer strings select clean or recent-turn forks. Full-history forks inherit model and effort. Each child has independent durable execution ownership and context-window identity while retaining its inherited cache prefix.
