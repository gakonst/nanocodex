...
mod tests {
    ...
    #[test]
    fn codex_commands_preserve_pdf_and_sports_requests() {
        let input = json!({
            "screenshot": [{"ref_id": "paper", "pageno": 2}],
            "sports": [{"tool": "sports", "fn": "schedule", "league": "nba"},
                       {"fn": "standings", "league": "nfl"}],
            "search_query": [{"q": "1"}, {"q": "2"}, {"q": "3"}, {"q": "4"}, {"q": "5"}]
        });
        let commands: SearchCommands = serde_json::from_value(input.clone()).unwrap();
        assert_eq!(serde_json::to_value(commands).unwrap(), input);
    }
