// Owned two-window receiver for the public companion isolation test.
import AppKit
...
for i in 0..<2 {
    let w = NSWindow(contentRect: NSRect(x: 60 + i * 400, y: 60, width: 360, height: 240), styleMask: [.titled], backing: .buffered, defer: false)
    w.title = "Owned window \(i)"
    let editor = NSTextView(frame: NSRect(x: 0, y: 0, width: 360, height: 240))
