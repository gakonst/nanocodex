...
const send=value=>{const sendingPort=port;if(!sendingPort)return false;try{const bytes=new TextEncoder().encode(JSON.stringify(value));if(bytes.length>8*1024*1024)throw new Error('Message exceeds 8 MiB');if(bytes.length<=512*1024){sendingPort.postMessage(value);return true;}const id=++chunkId,total=Math.ceil(bytes.length/(256*1024));for(let part=0;part<total;part++){const chunk=bytes.subarray(part*256*1024,(part+1)*256*1024);let text='';for(let i=0;i<chunk.length;i+=32768)text+=String.fromCharCode(...chunk.subarray(i,i+32768));sendingPort.postMessage({__skyre_chunk:{id,part,total,data:btoa(text)}});}return true;}catch{disconnectPort(sendingPort);return false;}};
function receive(value){if(!value.__skyre_chunk){if(pendingChunk)throw new Error('Interleaved chunks');return value;}const c=value.__skyre_chunk;if(!Number.isSafeInteger(c.id)||!Number.isInteger(c.part)||!Number.isInteger(c.total)||c.total<1||c.total>64)throw new Error('Invalid chunk');if(!pendingChunk){if(c.part!==0)throw new Error('Missing chunk');pendingChunk={id:c.id,total:c.total,part:0,bytes:[],length:0,time:Date.now()};}const p=pendingChunk;if(p.id!==c.id||p.total!==c.total||p.part!==c.part||Date.now()-p.time>10000)throw new Error('Stale chunk');const bytes=Uint8Array.from(atob(c.data),c=>c.charCodeAt(0));if(bytes.length>256*1024||p.length+bytes.length>8*1024*1024)throw new Error('Chunk budget');p.bytes.push(bytes);p.length+=bytes.length;p.part++;if(p.part<p.total)return undefined;const out=new Uint8Array(p.length);let offset=0;for(const bytes of p.bytes){out.set(bytes,offset);offset+=bytes.length;}pendingChunk=undefined;return JSON.parse(new TextDecoder('utf-8',{fatal:true}).decode(out));}
