...
struct Fixture { directory: PathBuf, fail_download: bool, bad_copy: bool, calls: Vec<String> }
impl Fixture {
    fn new() -> Self {
        let directory = std::env::temp_dir().join(format!("nanocodex-upstream-test-{}", nonce()));
        fs::create_dir_all(&directory).unwrap();
        Self { directory, fail_download: false, bad_copy: false, calls: Vec::new() }
    }
    fn app(path: &Path) {
        for relative in ["Contents/Info.plist".to_owned(), format!("{RESOURCES}/codex"), format!("{RESOURCES}/cua_node/bin/node"), format!("{RESOURCES}/cua_node/bin/node_repl"), format!("{RESOURCES}/{MODULES}/{ENTRY}"), format!("{RESOURCES}/{MODULES}/@oai/sky/package.json"), format!("{RESOURCES}/{MODULES}/@oai/browser-desktop/package.json"), format!("{RESOURCES}/{MODULES}/{SKY}/Contents/MacOS/SkyComputerUseService")] {
            let file = path.join(relative); fs::create_dir_all(file.parent().unwrap()).unwrap(); fs::write(file, "signed fixture").unwrap();
        }
    }
    fn copy(source: &Path, target: &Path) {
        fs::create_dir_all(target).unwrap();
        for entry in fs::read_dir(source).unwrap() {
            let entry = entry.unwrap(); let target = target.join(entry.file_name());
            if entry.file_type().unwrap().is_dir() { Self::copy(&entry.path(), &target); }
    ...
    fn run(&mut self, program: &str, args: &[OsString]) -> Result<String, String> {
        ...
        match program {
            "/usr/bin/codesign" => {
                ...
                if app.join("tampered").exists() { return Err("invalid signature".into()); }
                if args[0] == "--verify" {
                    assert!(args.iter().any(|a| a == "--deep"));
                    assert!(args.iter().any(|a| a == REQUIREMENT));
                    Ok(String::new())
                } else { Ok(format!("TeamIdentifier={TEAM}\nIdentifier={BUNDLE}\n")) }
            }
            "/usr/libexec/PlistBuddy" => Ok(if args[1] == "Print :CFBundleIdentifier" { BUNDLE } else { "1234" }.into()),
            "/usr/sbin/sysctl" => Ok("1".into()),
            "/usr/bin/curl" => {
                assert_eq!(args.last().unwrap(), ARM_DMG);
                assert!(args.iter().any(|a| a == "--proto-redir"));
                if self.fail_download { Err("network unavailable".into()) } else { Ok(String::new()) }
            }
            "/usr/bin/hdiutil" => {
                if args[0] == "attach" {
            ...
            "/usr/bin/ditto" => {
                ...
                if self.bad_copy { fs::write(PathBuf::from(&args[1]).join("tampered"), "changed").unwrap(); }
                Ok(String::new())
            }
...
fn installed_app_is_copied_verified_and_reused_without_network() {
    let mut f = Fixture::new(); let applications = f.directory.join("Applications");
    ...
    assert_eq!(receipt["executable"], root.join("current/cua-provider").to_str().unwrap());
    ...
    assert!(Command::new("/bin/sh").args(["-n"]).arg(root.join("current/cua-provider")).status().unwrap().success());
    f.calls.clear();
    assert_eq!(provision(&root, &[], false, &mut f).unwrap(), receipt);
    assert!(!f.calls.iter().any(|s| s.ends_with("curl") || s.ends_with("ditto")));
}

#[test]
fn clean_install_downloads_and_detaches_before_atomic_selection() {
    let mut f = Fixture::new(); let root = f.directory.join("runtime");
...
fn failed_refresh_preserves_previous_selection_and_success_keeps_old_version() {
    ...
    f.fail_download = false; f.bad_copy = true;
    assert!(provision(&root, &[], true, &mut f).unwrap_err().contains("signature"));
    assert_eq!(fs::read_link(root.join("current")).unwrap(), previous);
    f.bad_copy = false;
...
fn corrupted_cache_requires_explicit_repair_instead_of_another_backend() {
    ...
    f.calls.clear();
    assert!(provision(&root, &[], false, &mut f).unwrap_err().contains("--refresh"));
    assert!(f.calls.is_empty());
    provision(&root, &[], true, &mut f).unwrap();
    fs::write(root.join("current/Codex.app/tampered"), "modified").unwrap();
    assert!(provision(&root, &[], false, &mut f).unwrap_err().contains("signature"));
}
