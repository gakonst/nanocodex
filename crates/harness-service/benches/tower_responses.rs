...
use harness_service::EncodedRequest;
...
fn retained_agent_event_trace(criterion: &mut Criterion) {
    let Some(path) = std::env::var_os("HARNESS_BENCH_EVENTS") else {
        eprintln!("HARNESS_BENCH_EVENTS is unset; skipping retained event-trace benchmarks");
        return;
    };
    ...
    let encoded = fs::read(&path).expect("HARNESS_BENCH_EVENTS should name a readable JSONL trace");
