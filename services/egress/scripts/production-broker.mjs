...
export function productionBrokerSecrets(environment) {
  ...
  const slackClientId = optional(environment.NANOCODEX_SLACK_OAUTH_CLIENT_ID,
    "NANOCODEX_SLACK_OAUTH_CLIENT_ID");
  const slackClientSecret = optional(environment.NANOCODEX_SLACK_OAUTH_CLIENT_SECRET,
  ...
  if ((slackClientId === undefined) !== (slackClientSecret === undefined)) {
    throw new Error("Slack OAuth application credentials must be configured together");
  }
  ...
  if (slackClientId !== undefined && slackClientSecret !== undefined) {
    secrets.SLACK_OAUTH_CLIENT_ID = slackClientId;
    secrets.SLACK_OAUTH_CLIENT_SECRET = slackClientSecret;
  }
