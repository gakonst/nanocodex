...
# macOS invokes the installed guided setup helper with the same custom root.
...
run_mac_cua_case() {
  ...
  local output status=0 setting="" helper_exit=0
  mkdir -p "$fixture" "$case_root/home"
  cat > "$fixture/nanocodex-aarch64-apple-darwin" <<'HELPER'
#!/bin/sh
printf '%s\n' "$NANOCODEX_DIR" "$@" > "$CUA_SETUP_RECORD"
exit "$CUA_SETUP_EXIT"
HELPER
  printf '%s\n' '#!/bin/sh' 'exit 0' > "$fixture/nanocodex2-aarch64-apple-darwin"
  : > "$fixture/SHA256SUMS"
  ...
  case "$mode" in off) setting=off ;; explicit) setting=/custom/provider ;; failure) helper_exit=1 ;; esac
  output="$(PATH="$mock_bin:$PATH" HOME="$case_root/home" SHELL=/bin/bash \
    ...
    NANOCODEX_INSTALL_TTY=/dev/null CUA_SETUP_RECORD="$case_root/setup" CUA_SETUP_EXIT="$helper_exit" \
  ...
  if [[ "$mode" == off ]]; then
    ...
    if [[ "$mode" == failure ]]; then
      [[ "$status" != 0 ]]
      grep -Fq 'binaries are installed; resume setup with' <<<"$output"
    else
      [[ "$status" == 0 ]]
    fi
  fi
