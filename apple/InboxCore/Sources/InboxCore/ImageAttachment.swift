...
enum ImageAttachmentContent {
    ...
    static func original(_ attachment: MessageAttachment) -> [JSON] {
        ...
        return [.object(["type": .string("text"), "text": .string(prefix + encoded + "\nUse view_image on preview_path first: it is an oriented, model-viewable JPEG bounded to 2048 pixels and 2 MiB. The original at path preserves the full resolution and original bytes for tasks that need them; it may be too large or unsupported by image tools. A JPEG at preview_path is available for quick previews or formats unsupported by the image tool.")])]
    }
