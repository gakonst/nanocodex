...
final class AgentNotificationUITests: XCTestCase {
    ...
    func testRunningThreadsStaySilentAndActivityLinksStillOpen() {
        ...
        XCTAssertTrue(app.staticTexts["conversation-title:data"].waitForExistence(timeout: 20))
        XCTAssertTrue(app.staticTexts["conversation-title:data"].isHittable)
        ...
        XCTAssertTrue(app.staticTexts["conversation-title:inbox"].waitForExistence(timeout: 20))
        XCTAssertTrue(app.staticTexts["conversation-title:inbox"].isHittable)
        openNotifications()
        XCTAssertFalse(inbox.exists)
        app.activate()
        app.terminate()
