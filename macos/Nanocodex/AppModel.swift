...
final class AppModel: ObservableObject {
    ...
    func persistLayout() {
        ...
        guard let payload = try? layoutPayload() else { return }
        persistence = Task {
            try? await Task.sleep(for: .milliseconds(350))
            guard !Task.isCancelled else { return }
            do { try await runtime.request("saveLayout", [payload]) }
            catch { self.error = error.localizedDescription }
        }
    }
    private func layoutPayload() throws -> JSONValue {
        let layout = try JSONValue.encoded(TabLayout(tabs: tabs, activeTabId: activeTabID, tabPosition: tabPosition, theme: theme))
        guard case .object(var fields) = layout else { return layout }
        if let accountScope = state.accountScope { fields["accountScope"] = .string(accountScope) }
    ...
    func prepareToQuit() async {
        ...
        _ = try? await runtime.request("saveLayout", [try layoutPayload()])
        runtime.stop()
    }
