...
test("the World is one Rust subagent task tree with one embodied session per resident", () => {
  ...
  assert.doesNotMatch(worker, /residentBoots|createResidentAgent/);
  ...
  assert.match(worker, /`world-leader:\$\{residentId\}`/);
  assert.match(worker, /`world-resident:\$\{residentId\}`/);
  assert.match(worker, /every addressed resident is one retained child/);
  assert.match(worker, /Promise\.all\(residentCalls\(active, setup\)/);
  assert.match(worker, /purpose=delegate replacement/);
  ...
  assert.match(worker, /const turn = agent\.turn\.prompt\(\{ input: setupPrompt\(active\) \}\);\s*active\.turn = turn;\s*const result = await turn\.result\(\)/);
  assert.match(worker, /agent\.turn\.prompt\(\{ input: coordinatorPrompt\(active, residentAgents, setup\) \}\)/);
  ...
  assert.match(worker, /delegatedCallId\(context\) !== currentCallId/);
  assert.match(worker, /already bound to another task-tree agent/);
  assert.doesNotMatch(worker, /act\.call_id/);
  ...
  assert.match(worker, /MANDATORY GLOBAL REVIEW/);
  assert.match(worker, /active\.feedback\.size !== active\.addressed\.size/);
  assert.match(worker, /completed without the mandatory global review/);
  assert.match(worker, /position\/maintain is unavailable until the complete first wave acts/);
  assert.match(worker, /peers: Object\.freeze\(\[\.\.\.active\.feedback\]/);
  assert.match(worker, /if \(pending\.active\.feedback\.size !== pending\.active\.addressed\.size\) return/);
  assert.match(worker, /pending\.active\.firstWaveComplete = true/);
  assert.match(worker, /const setup = await planWorldSetup\(agent, active\)/);
  assert.match(worker, /active\.setup = setup;[\s\S]*?await dispatchInitialWave\(active, setup\);[\s\S]*?dispatchResidents\(agent, active, setup\)/);
  assert.match(worker, /async function dispatchInitialWave[\s\S]*?await Promise\.all\(residentCalls\(active, setup\)\.map[\s\S]*?postWorldAction/);
  assert.match(worker, /runtime dispatches the complete provisional movement wave immediately after setup/);
  assert.match(worker, /groupOrdinal: formationMemberIds\(active, setup\.get\(leader\)\)\.indexOf\(residentId\)/);
  assert.match(worker, /function plannedPositionAction/);
  assert.match(worker, /function sampleFormationPath/);
  assert.match(worker, /component \* FORMATION_EXTENT_PIXELS/);
  assert.match(worker, /new Set\(active\.setup\?\.values\(\) \?\? \[\]\)/);
  assert.match(worker, /active\.addressed\.has\(formation\.anchor as ResidentId\)/);
  assert.match(worker, /anchored a formation to a moving resident/);
