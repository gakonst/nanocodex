...
fn copy_tree(source: &Path, target: &Path) -> std::io::Result<()> {
    ...
    for entry in fs::read_dir(source)? {
        ...
        if entry.file_type()?.is_dir() { copy_tree(&entry.path(), &output)?; }
        else { fs::copy(entry.path(), output)?; }
    }
    Ok(())
}
...
async fn relocated_helper_initializes_negotiates_and_closes() -> anyhow::Result<()> {
    ...
    let helper = package.package_dir.join("nanocodex-resources/voice/bin").join(
        if cfg!(windows) { "nanocodex-voice-host.exe" } else { "nanocodex-voice-host" });
    let version = tokio::process::Command::new(helper).arg("--build-commit").output().await?;
    ...
    tokio::time::timeout(Duration::from_secs(40), async {
        let host = VoiceHost::connect(&package, &commit).await?.initialize_runtime().await?;
        let (host, offer) = host.start_transport().await?;
        anyhow::ensure!(offer.into_sdp().contains("m=audio"), "missing audio offer");
        host.close().await?;
        let host = VoiceHost::connect(&package, &commit).await?.initialize_runtime().await?;
        anyhow::ensure!(host.initialize_runtime().await.is_err(), "duplicate runtime accepted");
        anyhow::ensure!(VoiceHost::connect(&package, "wrong-build").await.is_err(), "incompatible build accepted");
        anyhow::Ok(())
    }).await??;
    Ok(())
}
