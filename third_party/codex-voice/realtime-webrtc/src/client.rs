...
impl VoiceHost {
    ...
    pub async fn connect(package: &VoicePackage, build_commit: &str) -> Result<Self> {
        ...
        let path = root.join("nanocodex-resources/voice/bin").join(name);
        ensure!(
        ...
        let SpawnedProcess {
            ...
            stderr_rx,
            exit_rx,
        } = crate::process::spawn_pipe_process(
            &path,
            &[],
            &root,
            &environment,
