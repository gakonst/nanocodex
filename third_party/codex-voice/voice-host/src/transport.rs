...
// Socket fixtures must stay on loopback instead of probing VPN/VM interfaces.
#[cfg(test)]
const BIND_ADDRESSES: &[&str] = &["127.0.0.1:0"];
#[cfg(not(test))]
const BIND_ADDRESSES: &[&str] = &["0.0.0.0:0", "[::]:0"];
    ...
    async fn with_runtime(runtime: Arc<dyn webrtc::runtime::Runtime>) -> Result<Self> {
        ...
        let connection: Arc<dyn PeerConnection> = Arc::new(
            PeerConnectionBuilder::new()
                ...
                .with_udp_addrs(BIND_ADDRESSES.to_vec())
                .with_tcp_addrs(BIND_ADDRESSES.to_vec())
