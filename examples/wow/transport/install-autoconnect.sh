...
lines=['[Unit]','Description=Nanocodex automatic WoW connection','After=graphical-session-pre.target network.target nanocodex-wow.service','PartOf=graphical-session.target','[Service]','Type=simple','WorkingDirectory='+quote(app),'UMask=0077',
